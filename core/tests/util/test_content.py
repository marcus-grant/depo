...
class TestDecodeBase64(TestCase):
    """Test cases for decode_base64 function"""

    def setUp(self):
        fx = fixtures
        self.test_bytes = [fx.PNG_DATA, fx.JPEG_DATA, fx.GIF_DATA]
        self.test_b64s = [
            fx.PNG_BASE64_DATA_URI,
            fx.JPEG_BASE64_DATA_URI,
            fx.GIF_BASE64_DATA_URI,
        ]
        self.test_mimes = ["image/png", "image/jpeg", "image/gif"]

    def test_valid_base64_str_returns_decoded_bytes(self):
        """Test valid base64 strings return correct bytes"""
        for input, expect, i in zip(self.test_b64s, self.test_bytes, range(3)):
            with self.subTest(case_num=i):
                result = content.decode_base64(input)
                self.assertIsNone(result.error)
                self.assertEqual(result.data, expect)
                self.assertEqual(result.mime, self.test_mimes[i])

    @patch("core.util.validator.valid_base64_format", return_value=False)
    def test_calls_base64_validator_on_invalid_data(self, mock_validator):
        """Test that decode_base64 calls validator and handles invalid base64"""
        invalid_b64 = "data:image/png;base64,invalid_base64_here!"
        result = content.decode_base64(invalid_b64)
        mock_validator.assert_called_once_with(invalid_b64)
